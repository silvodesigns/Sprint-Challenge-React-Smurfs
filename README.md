...
- [ ] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  `Service-side routing is handled by the server that is controlling the data that is painting to the DOM. Client-side routing is handled by the internal javascript running on the web application. Here the page wont refresh as it would by the server side routing.`
